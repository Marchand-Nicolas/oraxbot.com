  ...
  useEffect(() => {
    ...
    if (!token || token === "undefined") {
      ...
      if (code) {
        ...
        window.location.href = `https://discord.com/api/oauth2/authorize?client_id=812298057470967858&redirect_uri=https%3A%2F%2Foraxbot.com%2Fdashboard&response_type=code&scope=identify%20guilds&state=${encodeURIComponent(
          ...
          channels.result ? (
            <div className={styles.channelContainer}>
              ...
              {channels.result.map((channel, index) => (
                <button
                  onClick={() => {
                    fetch(`${config.serverIp}join_group_by_link`, {
                      method: "POST",
