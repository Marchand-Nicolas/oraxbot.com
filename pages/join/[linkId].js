    ...
    useEffect(() => {
        ...
        if (!token || token === 'undefined') {
            ...
            if (code) {
                ...
                window.location.href = `https://discord.com/api/oauth2/authorize?client_id=812298057470967858&redirect_uri=https%3A%2F%2Foraxbot.com%2Fdashboard&response_type=code&scope=identify%20guilds&state=${encodeURIComponent(window.location.href)}`
            }
        }
        else {
            loadPage()
    ...
    return <>
        ...
        <div className={styles.page}>
            ...
            channels.result ?
                <div className={styles.channelContainer}>
                    ...
                    {channels.result.map((channel, index) => 
                        ...
                        }} key={"channel_" + index} className={styles.channelButton}>{channel.name}</button>
                    )}
                </div>
            ...
            : <div className={styles.selectGuildContainer}>
                ...
                {
                    adminGuildNumber > 0 ? guilds.map(g =>
                        checkAdminPerms(g) ?
                            <Link key={"nav_guild_" + g.id} href={`./${linkId}?guild=${g.id}`}>
