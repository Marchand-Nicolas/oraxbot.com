...
export default function EmojisPage() {
  ...
  const [exact, setExact] = useState<Emoji[]>([]);
  const [similar, setSimilar] = useState<Emoji[]>([]);
  ...
  const fetchEmojiPage = useCallback(
    async ({
      ...
      try {
        ...
        const data: EmojiSearchResult = await res.json();
  ...
  const renderEmojiGrid = (items: Emoji[]) => {
    if (!items.length) {
        ...
        <section className={styles.searchSection}>
          ...
          <input
            ...
            onChange={(event: ChangeEvent<HTMLInputElement>) =>
              searchEmojis(event.target.value)
            }
